...
from vlnce_baselines_120.models.etp.ETP_R1_vlnbert_init import get_vlnbert_models
from vlnce_baselines_120.common.aux_losses import AuxLosses
from vlnce_baselines_120.models.encoders.instruction_encoder import (
    InstructionEncoder,
)
from vlnce_baselines_120.models.encoders.resnet_encoders import (
    TorchVisionResNet50,
    VlnResnetDepthEncoder,
    CLIPEncoder,
)
from vlnce_baselines_120.models.policy import ILPolicy

from vlnce_baselines_120.waypoint_pred.TRM_net import BinaryDistPredictor_TRM
from vlnce_baselines_120.waypoint_pred.utils import nms
from vlnce_baselines_120.models.utils import (
...
class ETP(Net):
    ...
    def forward(self, mode=None, 
        ...
        if mode == 'language':
            ...
            NUM_ANGLES = 120    # 4-camera: 120 angles, 3 degrees each
            ...
            if in_train:
                ...
                HEATMAP_OFFSET = 15  # 120/4/2 = 15
                batch_way_heats_regional = torch.cat(
                    (waypoint_heatmap_logits[:,-HEATMAP_OFFSET:,:], 
                    waypoint_heatmap_logits[:,:-HEATMAP_OFFSET,:],
                ), dim=1)
                batch_way_heats_regional = batch_way_heats_regional.reshape(batch_size, 4, 30, 12)  # 4-camera, 30 angles each
                ...
                for j in range(batch_size):
                    ...
                    img_idxes = ((angle_idxes.cpu().numpy()+15) // 30)  # 120/4 = 30 angles per image
                    ...
                    for k, way_act in enumerate(way_heats_act):
                        if img_idxes[k] != 0:
                            angle_pointer = (img_idxes[k] - 1) * 30 + 15  # 30 angles per image
                        else:
                            angle_pointer = 0
            ...
            for j in range(batch_size):
                ...
                # for angle & distance
                angle_rad_c = angle_idxes.cpu().float()/120*2*math.pi       # 4-camera: 120 angles, 顺时针
                ...
                # for img idxes
                img_idxes = 4 - (angle_idxes.cpu().numpy()+15) // 30        # 4-camera: 逆时针, 30 angles per image
                img_idxes[img_idxes==4] = 0  # 4-camera: wrap at 4
